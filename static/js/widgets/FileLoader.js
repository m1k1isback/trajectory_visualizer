...
function updateFileProperties(result) {
    ...
    html += '<tr><td style="color: #858585; padding: 2px;">Валиден:</td><td style="text-align: right; color: ' + (result.is_valid ? '#4ec9b0' : '#f44747') + ';">' + (result.is_valid ? 'Да' : 'Нет') + '</td></tr>';
    html += '</table>';
...
function updateStatistics(result) {
    ...
    for (const colName in result.statistics) {
        ...
        html += '<td style="padding: 6px; text-align: right; color: ' + (stats.nan_count > 0 ? '#f44747' : '#858585') + ';">' + stats.nan_count + '</td>';
        html += '</tr>';
    }
...
function updateDataTable(result) {
    ...
    result.data_sample.forEach(function(row) {
        ...
        row.forEach(function(cell) {
            html += '<td style="padding: 4px 6px; color: #cccccc;">' + formatNumber(cell) + '</td>';
        });
        html += '</tr>';
    });
    ...
    if (result.row_count > 50) {
        html += '<p style="color: #858585; font-size: 11px; margin-top: 8px;">Показано 50 из ' + result.row_count + ' строк</p>';
    }
